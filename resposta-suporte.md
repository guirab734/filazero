...
1. Abra o painel em outro dispositivo, outro computador, um notebook ou até um
   celular ou tablet ligado na TV. Se ele voltar a atualizar, a recepção segue
...
Separa um problema local (aquele equipamento, rede ou navegador) de um problema
na plataforma. Se só o painel da recepção parou, a investigação fica no
dispositivo; se tudo parou, é backend e a prioridade muda na hora, porque deixa
de ser um cliente afetado e passa a ser potencialmente todos.
...
painel mantém aberta). Se funciona no 4G e não no Wi-Fi da clínica, o problema é
a rede local, não a Filazero. E o cliente já disse que reiniciou o computador,
então esse teste é o que mais informação entrega pelo menor esforço dele.
...
- Assim que as verificações indicarem que o problema **não é local**, ou seja, o
  painel falha em outro dispositivo e em outra rede, ou a senha avança no
  sistema mas não na tela. Aí a causa está do nosso lado e o suporte não resolve
  sozinho.
- Se houver **qualquer sinal de que passa de um cliente**, como outros chamados
  parecidos no mesmo período. Nesse caso escalo imediatamente, mesmo sem ter
  terminado o diagnóstico, porque a suspeita de incidente coletivo vale mais que
  a investigação completa de um caso.
...
- cliente, unidade e identificador da fila ou painel;
...
mantenho o cliente informado a cada 30 minutos, mesmo sem novidade, porque
silêncio durante uma parada é o que mais desgasta a relação. E quando resolver,
volto ao cliente explicando o que aconteceu e o que evita a repetição.
